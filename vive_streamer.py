...
def update_vive_data(latest_vive_data, pose, role):
    """
    ...
    latest_vive_data[role] = data

frame_quat = Rotation.from_euler('xyz', [0, -25, 0], degrees=True)
frame_pos = np.array([-0.044, 0, 0.54])
def yup_zf_to_zup_xf(position: np.ndarray, quat_xyzw: np.ndarray):
    # Basis-change matrix: new = C * old
    C = np.array([
        [0, 0, -1],  # new X = old Z
        [-1, 0, 0],  # new Y = old X
        [0, 1, 0],  # new Z = old Y
    ...
    new_pos = C @ position
    # remap orientation
    r_old = Rotation.from_quat(quat_xyzw)        # scipy expects [x, y, z, w]
    R_new = C @ r_old.as_matrix() @ C.T   # change basis of rotation matrix
    r_new = Rotation.from_matrix(R_new)
    new_quat = r_new.as_quat()            # returns [x, y, z, w]
    return new_pos, new_quat
def to_head_frame(pos, orn):
    head_orn = Rotation.from_quat(orn) * frame_quat.inv()
    head_pos = pos + head_orn.apply(-frame_pos)
    return head_pos, head_orn.as_quat()
...
# @click.command()
# @click.option('--port', default=5555, help='port number')
# @click.option('--frequency', default=60, help='frequency')
def main(port, frequency, use_relative):
    with ContextObject(
            ...
            if context.session_state == xr.SessionState.FOCUSED:
                ...
                for index, space in enumerate(tracker_action_spaces):
                    ...
                    if space_location.location_flags & xr.SPACE_LOCATION_POSITION_VALID_BIT:
                        update_vive_data(latest_vive_data, space_location.pose, role_strings[index])
                        found_tracker_count += 1
                #print(latest_vive_data)
                # t_sleep = t_cycle_end - time.time()
                # iter_idx += 1
                # if t_sleep > 0:
                #     time.sleep(t_sleep)
                # else:
                ...
                dq.append(time.time())
                fps = (len(dq) - 1) / max(dq[-1] - dq[0], 1e-8)
                print(f'{found_tracker_count} trackers are found. Running fps = {fps:.2f}')
                    
            #if socket.poll(1):
                #request=socket.recv_string()
            #message = json.dumps(latest_vive_data).encode()
            #sock.sendto(message, ("10.112.210.141", 5555))
            pose = latest_vive_data["left_foot"]
            pos = np.array([pose["position"]["x"],pose["position"]["y"],pose["position"]["z"]])
            orn = np.array([pose["orientation"]["x"],pose["orientation"]["y"],pose["orientation"]["z"], pose["orientation"]["w"]])
            pos_zup, orn_zup = yup_zf_to_zup_xf(pos, orn)
            head_pos, head_orn = to_head_frame(pos_zup, orn_zup)
            # Implement recentering
            if init:
                init_pose = np.hstack([head_pos, head_orn]).copy()
                init = False
            if use_relative:
                head_pos[:2] -= init_pose[:2]
                heading = get_heading(init_pose[3:])
                heading_inv = Rotation.from_euler('z', -heading, degrees=False)
                head_pos = heading_inv.apply(head_pos)
                head_orn = (heading_inv*Rotation.from_quat(head_orn)).as_quat()
            head_pos[2] += Z_OFFSET
            redis_client.set("head_pos", pickle.dumps(head_pos))
            redis_client.set("head_quat", pickle.dumps(head_orn))
...
if __name__=='__main__':
    ...
    main(args.port, args.frequency, args.use_relative)